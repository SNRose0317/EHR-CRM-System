...
import type { Route, DoseForm } from '../../constants/medication-data';
...
export class RouteValidator {
  private static readonly ROUTE_ALIASES = new Map<string, string>([
    // Oral aliases
    ['oral', 'Orally'],
    ['po', 'Orally'],
    ['by mouth', 'Orally'],
   ...
   */
  private static validateRouteCompatibility(route: string, doseForm: string): RouteValidationResult {
    const result: RouteValidationResult = {
      isValid: false,
      errors: [],
      warnings: []
    ...
    if (!routeData.applicableForms.includes(doseForm)) {
      result.errors.push(
        `Route "${route}" is not applicable for dose form "${doseForm}". ` +
    ...
    if (!doseFormData.applicableRoutes.includes(route)) {
      result.errors.push(
        `Dose form "${doseForm}" does not allow route "${route}". ` +
   ...
   */
  private static getSuggestedRoutes(route: string): string[] {
    const normalized = route.toLowerCase().trim();
