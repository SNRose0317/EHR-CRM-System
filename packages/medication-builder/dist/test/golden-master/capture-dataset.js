 ...
 */
function generateSystematicCombinations() {
    const testCases = [];
 ...
 */
export async function captureGoldenDataset() {
    ...
    const goldenMasterRunner = createSignatureGoldenMasterRunner((medication, dose, route, frequency, specialInstructions) => {
 ...
 */
export function saveGoldenDataset(dataset, filePath) {
    const fs = require('fs');
 ...
 */
export function loadGoldenDataset(filePath) {
    const fs = require('fs');
...
export function validateGoldenDataset(dataset) {
    ...
    if (!dataset.metadata) {
        ...
        if (!dataset.metadata.captureDate)
            errors.push('Missing capture date');
        if (!dataset.metadata.legacySystemVersion)
            errors.push('Missing legacy system version');
    ...
    if (!Array.isArray(dataset.entries)) {
        ...
        dataset.entries.forEach((entry, index) => {
            ...
            if (!entry.input)
                errors.push(`Entry ${index}: Missing input`);
            if (!entry.output)
                errors.push(`Entry ${index}: Missing output`);
            if (!entry.output.humanReadable)
                errors.push(`Entry ${index}: Missing humanReadable output`);
            if (!entry.metadata)
                errors.push(`Entry ${index}: Missing metadata`);
        });
    }
...
//# sourceMappingURL=capture-dataset.js.map