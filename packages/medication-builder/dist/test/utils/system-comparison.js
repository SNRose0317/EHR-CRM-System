 ...
 * @since 3.1.0
 */
import { compareDoses } from '../matchers/clinical-matchers';
/**
 * Compare outputs from legacy and new systems
 */
export function compareSystemOutputs(testCase, legacyResult, newSystemResult) {
    const differences = [];
    let confidence = 1.0;
    let comparisonType = 'identical';
    // Handle error cases
    ...
    if (legacySignature === newSystemSignature) {
        ...
        const componentDifferences = analyzeComponentDifferences(legacyResult.actualOutput, newSystemResult.actualOutput, testCase);
 ...
 */
function analyzeComponentDifferences(legacyOutput, newSystemOutput, testCase) {
    const differences = [];
    // Dose analysis
 ...
 */
function extractRoute(signature) {
 ...
 */
function extractFrequency(signature) {
 ...
 */
function areFrequenciesEquivalent(freq1, freq2) {
    const equivalents = {
 ...
 */
function extractSpecialInstructions(signature) {
 ...
 */
function analyzeFhirStructure(legacyFhir, newSystemFhir) {
    const differences = [];
 ...
 */
function generateComparisonSummary(comparisonType, differences, confidence) {
 ...
 */
export function exportComparisonDocumentation(report) {
    return {
        executiveSummary: {
...
//# sourceMappingURL=system-comparison.js.map