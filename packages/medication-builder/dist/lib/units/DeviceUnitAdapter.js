import { ConversionErrors } from './ConversionErrors';
/**
 * Default device unit definitions for common medical devices
 */
const DEFAULT_DEVICE_UNITS = [
    {
        id: '{click}',
        display: 'click',
 ...
 */
export class DeviceUnitAdapter {
    constructor(ucumWrapper) {
        this.ucumWrapper = ucumWrapper;
        this.deviceUnits = new Map();
     ...
     */
    convertBetweenDeviceUnits(value, from, to, context, trace) {
        ...
        if (fromDevice.ratioTo !== toDevice.ratioTo) {
            ...
            const intermediate = this.convertFromDeviceUnit(value, from, intermediateUnit, context, trace);
            // Convert from intermediate to target device
            return this.convertToDeviceUnit(intermediate.value, intermediateUnit, to, context, trace);
        }
     ...
     */
    convertFromDeviceUnit(value, from, to, context, trace) {
     ...
     */
    convertToDeviceUnit(value, from, to, context, trace) {
     ...
     */
    getConversionFactor(deviceUnit, context) {
        // Check for custom conversions in context first
        if (context?.customConversions) {
            const custom = context.customConversions.find(c => c.from === deviceUnit.id && c.to === deviceUnit.ratioTo);
            if (custom) {
     ...
     */
    createMissingContextError(deviceUnit, context) {
        ...
        return ConversionErrors.missingContext(requiredContext, `${deviceUnit.id} conversion`, context);
    }
}
//# sourceMappingURL=DeviceUnitAdapter.js.map