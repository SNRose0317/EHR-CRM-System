...
import { ConversionError, ConversionErrors } from './ConversionErrors';
...
export class UnitConverter {
    constructor(ucumWrapper, deviceAdapter, confidenceService, tracerOptions) {
     ...
     */
    convert(value, from, to, context, options) {
        ...
        try {
            ...
            if (from === to) {
                ...
                const confidenceTrace = this.confidenceService.createTraceFromSteps(identityTrace, { value, fromUnit: from, toUnit: to }, {
                    usedDefaults: false,
            ...
            if (this.isConcentrationConversion(from, to, context)) {
                ...
                return this.handleConcentrationConversion(value, from, to, context, trace, opts);
            }
            ...
            const confidenceTrace = this.confidenceService.createTraceFromSteps(result.trace, { value, fromUnit: from, toUnit: to }, {
     ...
     */
    registerDeviceUnit(unit) {
        this.deviceAdapter.registerDeviceUnit(unit);
    }
    /**
     * Validate conversion inputs
     */
    validateInputs(value, from, to) {
        if (!isFinite(value)) {
            throw new Error('Value must be a finite number');
        }
        const fromValidation = this.validate(from);
        if (!fromValidation.valid) {
            throw ConversionErrors.invalidUnit(from, fromValidation.error, fromValidation.suggestions);
        }
        const toValidation = this.validate(to);
        if (!toValidation.valid) {
            throw ConversionErrors.invalidUnit(to, toValidation.error, toValidation.suggestions);
        }
    }
    /**
     * Check if conversion requires concentration context
     */
    isConcentrationConversion(from, to, context) {
     ...
     */
    handleConcentrationConversion(value, from, to, context, trace, options) {
        this.tracer.trace({
            type: 'conversion_step',
        ...
        if (!context?.strengthRatio) {
            throw ConversionErrors.missingContext(['strengthRatio'], `${from} to ${to} conversion`, context);
        }
        ...
        if (fromIsVolume) {
            ...
            if (from !== denominator.unit) {
                try {
                    const volumeConversion = this.deviceAdapter.convert(value, from, denominator.unit, context);
            ...
            if (numerator.unit !== to) {
                const finalConversion = this.deviceAdapter.convert(massValue, numerator.unit, to, context);
            ...
            if (from !== numerator.unit) {
                try {
                    const massConversion = this.deviceAdapter.convert(value, from, numerator.unit, context);
            ...
            if (denominator.unit !== to) {
                const finalConversion = this.deviceAdapter.convert(volumeValue, denominator.unit, to, context);
                trace.push(...finalConversion.trace);
                result = finalConversion.value;
            }
            else {
        ...
        // Calculate confidence score using the ConfidenceScoreService
        const confidenceTrace = this.confidenceService.createTraceFromSteps(trace, { value, fromUnit: from, toUnit: to }, {
     ...
     */
    checkPrecision(result, tolerance) {
        ...
        if (relativeError > tolerance) {
            throw ConversionErrors.precisionLoss(result.originalValue, result.fromUnit, result.toUnit, tolerance, relativeError);
        }
    }
    /**
