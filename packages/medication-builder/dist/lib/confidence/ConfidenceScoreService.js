...
export class ConfidenceScoreService {
    ...
    calculate(trace) {
        ...
        const adjustedScore = adjustments.reduce((score, adj) => score + adj.value, baseScore);
        // Clamp to 0-1 range
    ...
    getBaseScore(trace) {
        ...
        if (stepCount === 1)
            return 0.95; // Direct conversion
        if (stepCount === 2)
            return 0.85; // Single intermediate
        if (stepCount === 3)
            return 0.70; // Two intermediates
        return 0.50; // Complex chain
    }
    /**
     * Calculate all adjustments based on conversion characteristics
     */
    getAdjustments(trace) {
        const adjustments = [];
        ...
        const hasOnlyStandardUnits = trace.steps.every(step => step.type === 'standard');
     ...
     */
    calculateFactors(trace, adjustments) {
     ...
     */
    generateRationale(trace, adjustments) {
        const rationale = [];
     ...
     */
    generateExplanation(trace, finalScore, adjustments) {
        const level = this.getLevel(finalScore);
     ...
     */
    createTraceFromSteps(steps, request, context) {
        return {
            steps,
            request,
...
//# sourceMappingURL=ConfidenceScoreService.js.map