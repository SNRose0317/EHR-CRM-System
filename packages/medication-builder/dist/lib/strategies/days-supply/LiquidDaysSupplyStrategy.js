...
import { createDaysSupplyResult, CALCULATION_CONSTANTS } from './types';
import { calculationUtils } from './CalculationUtils';
export class LiquidDaysSupplyStrategy {
    constructor() {
        this.specificity = SpecificityLevel.DOSE_FORM;
        this.id = 'liquid-days-supply';
        this.name = 'Liquid Medication Days Supply Calculator';
        this.temporalParser = new FHIRTemporalParser();
        this.liquidDoseForms = ['solution', 'vial', 'suspension', 'syrup', 'elixir'];
    }
    /**
    ...
    calculate(context) {
        ...
        try {
            ...
            if (dosesPerDay === 0) {
                // PRN medication
                return createDaysSupplyResult(0, 'PRN/As-needed liquid medication', this.createBreakdown(context, 0, 0, []), CALCULATION_CONSTANTS.LOW_CONFIDENCE, ['Cannot calculate days supply for PRN medications']);
            }
            ...
            return createDaysSupplyResult(0, 'Liquid calculation failed', this.createBreakdown(context, 0, 0, []), CALCULATION_CONSTANTS.LOW_CONFIDENCE, [`Error: ${errorMessage}`]);
        }
    }
    /**
    ...
    calculateDosesPerDay(timing) {
        ...
        return calculationUtils.calculateDosesPerDay(repeat.frequency, repeat.period, repeat.periodUnit);
    }
    /**
     * Perform liquid-specific calculation
     */
    performLiquidCalculation(context, dosesPerDay) {
        const conversions = [];
        const warnings = [];
        ...
        const volumeStandardization = this.standardizeVolumeUnits(effectiveDoseVolume, effectiveDoseUnit, context.packageUnit);
        ...
        const breakdown = this.createBreakdown(context, dosesPerDay, consumptionPerDay, conversions);
        return createDaysSupplyResult(daysSupply, 'Liquid medication calculation with concentration handling', breakdown, confidence, warnings);
    }
    /**
     * Handle concentration-based conversions (weight to volume)
     */
    handleConcentrationConversion(context) {
        // Check if dose is in weight units but package is in volume
        const isWeightDose = ['mg', 'mcg', 'g', 'kg', 'unit', 'units'].includes(context.doseUnit.toLowerCase());
        const isVolumePackage = ['ml', 'l', 'fl oz', 'oz'].includes(context.packageUnit.toLowerCase());
     ...
     */
    standardizeVolumeUnits(amount, fromUnit, toUnit) {
     ...
     */
    validateLiquidPrecision(volumeAmount, context) {
        const warnings = [];
        let confidence = CALCULATION_CONSTANTS.HIGH_CONFIDENCE;
     ...
     */
    isCommonLiquidMeasurement(volume) {
        ...
        return commonMeasurements.some(measurement => Math.abs(volume - measurement) < tolerance);
    }
    /**
     * Create calculation breakdown
     */
    createBreakdown(context, dosesPerDay, consumptionPerDay, conversions) {
        return {
...
//# sourceMappingURL=LiquidDaysSupplyStrategy.js.map