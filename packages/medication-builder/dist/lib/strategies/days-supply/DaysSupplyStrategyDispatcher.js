...
import { createDaysSupplyResult, CALCULATION_CONSTANTS, DaysSupplyCalculationError } from './types';
 ...
 */
class DefaultDaysSupplyStrategy {
    constructor() {
        this.specificity = SpecificityLevel.DEFAULT;
        this.id = 'default-days-supply';
        this.name = 'Default Days Supply Calculator';
    }
    ...
    calculate(context) {
        ...
        if (dosesPerDay === 0) {
            return createDaysSupplyResult(0, 'PRN medication - days supply cannot be calculated', {
                ...
                dosesPerDay: 0,
                consumptionPerDay: 0
            }, CALCULATION_CONSTANTS.LOW_CONFIDENCE, ['PRN/as-needed medications cannot have days supply calculated']);
        }
        ...
        return createDaysSupplyResult(daysSupply, 'Basic calculation without specialized strategy', {
            packageQuantity: context.packageQuantity,
            packageUnit: context.packageUnit,
            doseAmount: context.doseAmount,
            doseUnit: context.doseUnit,
            dosesPerDay,
            consumptionPerDay
        }, CALCULATION_CONSTANTS.MEDIUM_CONFIDENCE, warnings);
    }
    estimateDosesPerDay(timing) {
        ...
        if (normalized.includes('four times daily') || normalized.includes('qid'))
            return 4;
        if (normalized.includes('three times daily') || normalized.includes('tid'))
            return 3;
        if (normalized.includes('twice daily') || normalized.includes('bid'))
            return 2;
 ...
 */
export class DaysSupplyStrategyDispatcher {
    constructor() {
        this.strategies = [];
    ...
    initializeStrategies() {
        ...
        this.strategies.push(new TitrationDaysSupplyStrategy(), new TabletDaysSupplyStrategy(), new LiquidDaysSupplyStrategy(), new DefaultDaysSupplyStrategy() // Always last as fallback
        );
    }
    /**
     * Selects and executes the most appropriate strategy
     */
    calculateDaysSupply(context) {
        const startTime = Date.now();
        try {
            // Validate input context
            const validationErrors = calculationUtils.validateInputs(context);
            if (validationErrors.length > 0) {
                throw new DaysSupplyCalculationError(`Invalid input context: ${validationErrors.join(', ')}`, context);
            }
            ...
            throw new DaysSupplyCalculationError(`Dispatcher error: ${error instanceof Error ? error.message : String(error)}`, context, 'DaysSupplyStrategyDispatcher');
        }
    }
    /**
    ...
    getStrategy(context) {
        ...
        const matchingStrategies = this.strategies.filter(strategy => strategy.matches(context));
        ...
        const sortedStrategies = matchingStrategies.sort((a, b) => b.specificity - a.specificity);
    ...
    addStrategy(strategy) {
        ...
        const insertIndex = this.strategies.findIndex(s => s.specificity < strategy.specificity);
        if (insertIndex === -1) {
...
//# sourceMappingURL=DaysSupplyStrategyDispatcher.js.map