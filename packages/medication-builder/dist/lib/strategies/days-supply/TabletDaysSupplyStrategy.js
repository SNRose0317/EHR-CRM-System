...
export class TabletDaysSupplyStrategy {
    constructor() {
        this.specificity = SpecificityLevel.DOSE_FORM;
        this.id = 'tablet-days-supply';
        this.name = 'Tablet/Capsule Days Supply Calculator';
        this.temporalParser = new FHIRTemporalParser();
        this.solidDoseForms = ['tablet', 'capsule', 'odt', 'troche'];
    }
    /**
    ...
    calculate(context) {
        ...
        try {
            ...
            if (dosesPerDay === 0) {
                ...
                return createDaysSupplyResult(0, 'PRN/As-needed medication', this.createBreakdown(context, 0, 0, []), CALCULATION_CONSTANTS.LOW_CONFIDENCE, ['Cannot calculate days supply for PRN medications']);
            }
            ...
            return createDaysSupplyResult(0, 'Calculation failed', this.createBreakdown(context, 0, 0, []), CALCULATION_CONSTANTS.LOW_CONFIDENCE, [`Error: ${errorMessage}`]);
        }
    }
    /**
     ...
     */
    performCalculation(context, dosesPerDay) {
        const conversions = [];
        const warnings = [];
        ...
        const breakdown = this.createBreakdown(context, dosesPerDay, consumptionPerDay, conversions, totalQuantity);
        return createDaysSupplyResult(daysSupply, 'Standard tablet/capsule calculation', breakdown, confidence, warnings);
    }
    /**
     ...
     */
    handleStrengthConversion(context) {
        ...
        const isTabletPackage = ['tablet', 'capsule', 'cap', 'tab'].includes(context.packageUnit.toLowerCase());
     ...
     */
    validateFractionalDosing(tabletCount, context) {
        const warnings = [];
        let confidence = CALCULATION_CONSTANTS.HIGH_CONFIDENCE;
     ...
     */
    isCommonTabletFraction(count) {
        const commonFractions = [0.25, 0.5, 0.75, 1, 1.25, 1.5, 1.75, 2, 2.5, 3];
        const tolerance = CALCULATION_CONSTANTS.PRECISION_TOLERANCE;
        return commonFractions.some(fraction => Math.abs(count - fraction) < tolerance);
    }
    /**
     * Create calculation breakdown
     */
    createBreakdown(context, dosesPerDay, consumptionPerDay, conversions, totalQuantity) {
        return {
...
//# sourceMappingURL=TabletDaysSupplyStrategy.js.map