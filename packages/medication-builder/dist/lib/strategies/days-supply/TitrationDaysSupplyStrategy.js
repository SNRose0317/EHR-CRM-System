 ...
 * @since 3.1.0
 */
import { isFHIRTiming } from '../../temporal/types';
import { FHIRTemporalParser } from '../../temporal/FHIRTemporalParser';
import { SpecificityLevel } from '../types';
import { createDaysSupplyResult, CALCULATION_CONSTANTS, InvalidTitrationScheduleError } from './types';
import { calculationUtils } from './CalculationUtils';
export class TitrationDaysSupplyStrategy {
    constructor() {
        this.specificity = SpecificityLevel.DOSE_FORM_AND_INGREDIENT;
        this.id = 'titration-days-supply';
        this.name = 'Titration Days Supply Calculator';
        this.temporalParser = new FHIRTemporalParser();
    }
    /**
    ...
    calculate(context) {
        ...
        try {
            ...
            if (validationErrors.length > 0) {
                throw new InvalidTitrationScheduleError(`Invalid inputs: ${validationErrors.join(', ')}`, context);
            }
            // Parse titration phases
            const phases = this.parseTitrationPhases(context);
            if (phases.length === 0) {
                throw new InvalidTitrationScheduleError('No valid titration phases found', context);
            }
            ...
            throw new InvalidTitrationScheduleError(`Titration calculation failed: ${errorMessage}`, context);
        }
    }
    /**
     * Parse titration phases from context
     */
    parseTitrationPhases(context) {
        ...
        if (Array.isArray(context.timing)) {
            if (typeof context.timing[0] === 'string') {
                ...
                return this.convertFHIRTimingsToPhases(context.timing, context);
            }
        }
        if (typeof context.timing === 'string') {
     ...
     */
    convertFHIRTimingsToPhases(timings, context) {
     ...
     */
    calculateTitrationDaysSupply(context, phases) {
        ...
        for (let i = 0; i < phases.length; i++) {
            ...
            const phaseResult = this.calculatePhaseSupply(phase, remainingQuantity, context);
        ...
        return createDaysSupplyResult(totalDays, 'Multi-phase titration calculation', breakdown, CALCULATION_CONSTANTS.HIGH_CONFIDENCE, warnings);
    }
    /**
     * Calculate supply for a single phase
     */
    calculatePhaseSupply(phase, availableQuantity, context) {
        const timing = phase.timing.repeat;
        if (!timing?.frequency || !timing?.period || !timing?.periodUnit) {
            throw new InvalidTitrationScheduleError(`Invalid timing in phase: ${phase.description}`, context);
        }
        // Calculate doses per day for this phase
        const dosesPerDay = calculationUtils.calculateDosesPerDay(timing.frequency, timing.period, timing.periodUnit);
        ...
        const consumptionResult = calculationUtils.calculateConsumptionPerDay(effectiveDoseAmount, phase.doseUnit, context.packageUnit, dosesPerDay, context.medication || {});
        let phaseDays = 0;
     ...
     */
    getEffectiveDoseAmount(phase, context) {
     ...
     */
    calculatePhaseDurationInDays(phase) {
        if (phase.duration.value === Infinity) {
            return 0; // Maintenance phase
        }
        return calculationUtils.convertDurationToDays(phase.duration.value, phase.duration.unit);
    }
    /**
     * Get maintenance phase information
     */
    getMaintenancePhaseInfo(phases, breakdowns) {
        const maintenancePhase = phases.find(p => p.isMaintenancePhase);
        if (!maintenancePhase) {
            return undefined;
        }
        const maintenanceBreakdown = breakdowns.find(b => b.phaseIndex === maintenancePhase.phaseIndex);
...
//# sourceMappingURL=TitrationDaysSupplyStrategy.js.map