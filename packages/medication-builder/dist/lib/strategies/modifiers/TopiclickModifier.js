 ...
 * @since 3.0.0
 */
export class TopiclickModifier {
    constructor() {
        this.priority = 10; // Early priority to modify dose display
        this.metadata = {
    ...
    appliesTo(context) {
        ...
        if (!medication)
            return false;
        const isDoseForm = ['cream', 'gel'].includes(medication.doseForm?.toLowerCase() || '');
     ...
     */
    modify(instruction, context) {
        const { dose, medication } = context;
        if (!dose || !instruction.text)
            return instruction;
        ...
        if (dose.unit === 'click' || dose.unit === 'clicks') {
            ...
            if (medication?.ingredient?.[0]?.strengthRatio) {
                ...
                modifiedText = modifiedText.replace(dosePattern, `${dose.value} ${unitText} (${mgValue.toFixed(1)} ${strengthRatio.numerator.unit})`);
            }
            else {
                // Just show clicks and mL
                const dosePattern = new RegExp(`${dose.value}\\s*${dose.unit}`, 'i');
                modifiedText = modifiedText.replace(dosePattern, `${dose.value} ${unitText} (${mlValue} mL)`);
            }
        }
        // Add Topiclick instruction if not already present
        if (!modifiedText.includes('Topiclick')) {
            modifiedText = modifiedText.replace(/\.$/, ' using Topiclick dispenser.');
        }
...
//# sourceMappingURL=TopiclickModifier.js.map