 ...
 * @since 3.0.0
 */
export class StrengthDisplayModifier {
    constructor() {
        this.priority = 20; // After dose conversions but before final formatting
        this.metadata = {
    ...
    modify(instruction, context) {
        ...
        const dosePattern = new RegExp(`(${dose.value}\\s+(?:and\\s+\\d+/\\d+\\s+)?${unitPattern})(?!\\s*\\()`, 'gi');
        // Add strength in parentheses
        modifiedText = modifiedText.replace(dosePattern, `$1 (${totalStrength} ${strengthUnit})`);
        return {
            ...instruction,
            text: modifiedText
        };
...
//# sourceMappingURL=StrengthDisplayModifier.js.map