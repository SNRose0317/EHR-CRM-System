...
export function createTemplateEngine(locale = 'en-US') {
    const engine = new TemplateEngine({
        locale,
        enablePerformanceLogging: process.env.NODE_ENV === 'development'
    });
    // Register all standard templates
    const standardTemplates = TEMPLATES[locale];
    if (standardTemplates) {
        Object.entries(standardTemplates).forEach(([key, template]) => {
            engine.registerTemplate(locale, key, template);
        });
    }
    // Register specialized templates if available
    const specializedTemplates = SPECIALIZED_TEMPLATES[locale];
...
export function validateTemplate(templateKey, sampleData) {
    try {
        const engine = createTemplateEngine();
        const result = engine.render(templateKey, sampleData);
